...
## Review log (revision 3, 2026-09-24 — orchestrator rulings after the build)
Build: steps 0–8 + polish committed (b774cbe … 859cebb). Blind critics: AFTER 48 pairs, BEFORE 14, TIE 12 of 77 (63% of votes). Regression: no regressions, §7 all clean.
Rulings on the gates the builders flagged (each was measured; the literal wording was unreachable or measured the world, not the camera):
- **A1** adopts the §5.3 reading: a ray whose blocker §5.3 leaves in the picture (camvis amber reason `nearBody` inside the 1 u the cut plane keeps, or `terrain`) counts as seen; `summary.m1.unexcused` must be empty. The Great Cupcake shells are now `noGhost` (architecture.js finish(), 2026-09-24), so `occ_silhouette` is expected to read 1.0 on the next run.
- **A2** raw target = the camvis baseline (`renders.noindex/camvis/baseline.json`, copy at `tools/_tmp/camvis_baseline.json`): mode 1 raw mean ≥ 0.877 over the 13 / 0.827 over P, and a probe may read below its baseline only while `cutK ≥ 0.5` there (the window carries the frame by design once §5.2 releases the old lift). The seed probe's 0.908 is retired.
- **A6 away** is read on the plane through the feet along the travel line (`flatEdge` ≥ 22 u); the along-terrain figure is informational. A6 toward stays ≥ 14 u (reads 15.6).
- **A11** = the proposed reading (rows with `occLift == 0` and `occDist` within 0.05 of goal; the step-9 spring arm at `occ_whisker_night` is intended).
- **A12** = camera fields only: two `snap()`s equal, and camvis rows/scripts identical after stripping rendered-world fields (sky %, frame hashes, triangle counts vary with other owners' live edits).
- **A13** stays a FAIL to be fixed in the frame-rate pass (Contract J): sweep frames p95 14.6 ms on hardware GL (limit 6 ms); the first candidate is tested regardless of the triangle budget. Also include `?cut=0` in the mobile measurement (the patch's `discard` costs early-z on tile GPUs).
- Ratified deviations: m2Pitch 0.15 + `m2PitchCap 0.58 m2SkyMin 0.045`; strafe/backpedal judged by the geometric band (s7 ≥ 4%), rendered sky on the street frames; `look(o).pan` is [right, forward] in the look basis; the V look's lift is `lookElevLook`; on a vehicle/flying only 1/2/3, boarding and landing end the look; density's mode-2 elevation held inside the sky-band room; the unpatched-blocker dolly is kept while the window carries the frame; local collider radius max(40, reach + 12); snap() starts density from 0; occlude()'s dolly only alongside a sweep-backed dolly/tilt.
- Applied by the orchestrator today: ui.js help rows (V, Q/E, 1 2 3), player.js header, powerups.js + flyer.js read `camera.current.azimuth`. Open for other owners: cave.js enterCave aim-before-show; visitor.js silhouette over his own body; decal.js foot pool in the cave floor; ui.js prompt hidden while looking + prompt dodges the visitor.
